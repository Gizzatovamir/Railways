...
class Tester:
    ...
    def test_switches(self) -> tuple:
        for switch in self.switches:
            cur_line = self.find_path_class.find_cur_line(switch['points'], switch["segments"],
                                                          constants=self.constants,
                                                          line_point=switch['line_point'])
            if cur_line == switch['line']:
                self.result.append("Matching on switch {} is successful".format(switch['id']))
                self.coincidence_counter += 1
            else:
                self.result.append("Matching on switch {} is unsuccessful".format(switch['id']))
        print("total successful matches is {} out of {}".format(self.coincidence_counter, len(self.switches)))
