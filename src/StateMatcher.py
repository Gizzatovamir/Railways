...
class StateMatcher:
    def __init__(self, kwargs: dict):
        rail_lines = RailLines(
            path_to_lines=kwargs["lines_path"], path_to_points=kwargs["points_path"]
        )
        ...
        self.initial_dict = {}
        self.result = []
        self.find_path_class = SwitchClass(int(kwargs["method"]), kwargs["mode"])
        self.switch_id = 0
        self.point_buffer = []
        ...
        for i in range(len(self.lines)):
            for j in range(len(self.lines[i]["points"]) - 1):
                ...
                if current_dict["dist"] < min_dist and current_dict["is_ortho"]:
                    ...
                    points = {
                        "gps_point": self.gps_points[index]["coords"],
                        "cur_line": line_points,
                    }
        if points:
            if all([not line_point["cross"] for line_point in points["cur_line"]]):
                points["gps_point"] = point_to_segment_projection(
                    points["gps_point"], points["cur_line"]
                )
    ...
    def find_segments_from_point(self, point: dict) -> list:
        ...
        for i in range(len(self.lines)):
            if (
                point["id"] in self.lines[i]["points"]
                and point["id"] != self.lines[i]["points"][-1]
            ):
                result.append(
                    [utils.find_dict(self.points, x) for x in self.lines[i]["points"]]
                )
        return result
    ...
    def find_segments_from_point_left(self, point: dict) -> list:
        ...
        for i in range(len(self.lines)):
            if (
                point["id"] in self.lines[i]["points"]
                and point["id"] != self.lines[i]["points"][0]
            ):
                result.append(
                    [
                        utils.find_dict(self.points, x)
                        for x in self.lines[i]["points"][::-1]
                    ]
                )
        return result
    ...
    def draw_full_map(self, new_points: list) -> None:
        ...
        ax.set_aspect("equal")
        RailLines.draw_lines(self.lines, self.points, ax, end_r=self.end_r)
    ...
    def add_point_to_result(
        ...
        if next_seg_dist["is_ortho"]:
            ...
            return [
                [[point, point_to_segment_projection(point["coords"], next_segment)]],
                break_condition,
            ]
        else:
            if (
                ortho_point_dist["cur_line"][0]["end"]
                or ortho_point_dist["cur_line"][1]["end"]
            ):
                break_condition = True
            else:
                break_condition = False
            if ortho_point_dist["is_ortho"]:
                new_point = point_to_segment_projection(
                    point["coords"], ortho_point_dist["cur_line"]
                )
    ...
    def find_next_segment(self, line_point: dict, condition: bool) -> list:
        ...
        for line in self.lines:
            if line_point["id"] in line["points"]:
                if (
                    line_point["id"] != line["points"][0]
                    and line_point["id"] != line["points"][-1]
                ):
                    next_line_point = self.find_next_point_in_line(
                        line_point, line["points"], condition
                    )
                    return [line_point, next_line_point]
                else:
                    if condition:
                        next_line_point = self.find_new_line_from_point_right(
                            line_point
                        )
                        return [line_point, next_line_point]
                    else:
    ...
    def find_next_point_in_line(
        self, line_point: dict, original_list: list, condition: bool
    ) -> dict:
        ...
        for i in range(len(original_list)):
            if line_point["id"] == original_list[i]:
                return utils.find_dict(
                    self.points, original_list[i + 1 if condition else i - 1]
                )
        ...
        for line in self.lines:
            if line_point["id"] in line["points"]:
                ...
                if 0 < index < len(line["points"]) - 1:
                    new_end_point = utils.find_dict(
                        self.points,
                        line["points"][index + 1 if condition else index - 1],
                    )
                    return [line_point, new_end_point]

    def get_last_valuable_segment_in_line(self, line: list, center: dict) -> list:
        observed_segments = line[:2]
        try:
            for i in range(1, len(line) - 1):
                if (center["coords"] - line[i]["coords"]).norm <= self.end_r:
                    observed_segments = [line[i], line[i + 1]]
                # if i == len(line) - 1:
                #     received_segments = self.find_segments_from_point(line[i + 1])
                #     is_valid_condition = utils.is_switch_valid(
                #         observed_segments, received_segments
                #     )
                #     if is_valid_condition["false"]:
                #         print("achieved end of segment chain")
                #         observed_segments = is_valid_condition["line"]
    ...
    def consider_segments(self, segments: list) -> list:
        ...
        """
        observed_segments = []
        line_point = segments[0][0]
        # print(line_point, " - center point on switch")
        try:
            observed_segments.append(
                self.get_last_valuable_segment_in_line(segments[0], line_point)
            )
            observed_segments.append(
                self.get_last_valuable_segment_in_line(segments[1], line_point)
            )
        except IndexError:
            return observed_segments
        # self.consider_segments()
        # print(len(observed_segments))
        return observed_segments

    def add_points_to_result_on_switch(
        self, points: list, cur_line: list, condition: bool
    ) -> tuple:
        """
        ...
        for point in points if condition else points[::-1]:
            ortho_point_dist = point_to_segment_distance(point["coords"], cur_line)
            if ortho_point_dist["is_ortho"]:
                result.append(
                    [point, point_to_segment_projection(point["coords"], cur_line)]
                )
            else:
                next_segment = self.find_next_segment(
                    cur_line[0 if ortho_point_dist["line_point"] else 1],
    ...
    def get_segments_on_switch(
        self,
        i: int,
        line_point: dict,
        last_line_ortho_point_dist: dict,
        condition: bool,
    ) -> dict:
        segments = (
            self.find_segments_from_point(line_point)
            if condition
            else self.find_segments_from_point_left(line_point)
        )
            ...
            result = [
                [
                    ...
                    point_to_segment_projection(
                        ...
                        last_line_ortho_point_dist["cur_line"],
                    ),
                ]
            ]
            return {
                "res": [
                    result,
                    True if segments[0][1]["end"] or segments[0][0]["end"] else False,
                    0,
                ],
        ...
        return {"segments": observed_segments, "is_valid_switch": True}

    def get_result_on_switch(
        self, i: int, line: list, last_line_ortho_point_dist: dict, condition: bool
    ) -> list:
        ...
        received_segments = self.get_segments_on_switch(
            i, line[1], last_line_ortho_point_dist, condition
        )
        ...
        # [print(line_point["id"]) for line_point in line]
        for segment in observed_segments:
            norm_from_center = (line_point["coords"] - segment[1]["coords"]).norm
            if norm_from_center < self.end_r:
                print(
                    "changed max R from {} to {}".format(self.end_r, norm_from_center)
                )
                self.end_r = norm_from_center
        while (
            self.gps_points[i]["coords"] - line_point["coords"]
        ).norm <= self.end_r and i < len(self.gps_points) - 1:
            points.append(self.gps_points[i])
            # print(i, " i index on points")
        ...
        if not is_valid_condition["is_valid"]:
            ...
            result, tmp_line = self.add_points_to_result_on_switch(
                points, self.initial_dict["cur_line"], condition
            )
            return [
                result,
                True
                if is_valid_condition["line"][0]["end"]
                or is_valid_condition["line"][1]["end"]
                else False,
                len(points),
            ]
    ...
    def add_point_on_cross(
        ...
        Args:
            ...
            ortho_point_dist: dictionary obout distance, flag is_ortho (is it possible to
                lower the height on segment), e.t.c
            condition: bool that demonstrates end or start of segment (True if start and False if end)
        ...
        if (self.gps_points[i]["coords"] - line[1]["coords"]).norm <= self.end_r:
            ...
            new_points, break_condition = self.add_point_to_result(
                self.gps_points[i], line[1], ortho_point_dist, condition
            )
    ...
    def match(self) -> None:
        ...
        while i < len(self.gps_points):
            step = 1
            p1, p2 = self.initial_dict["cur_line"][:2]
            ...
            if ortho_point_dist["is_ortho"]:
                ...
                if p2["cross"] if not ortho_point_dist["line_point"] else p1["cross"]:
                    new_points, break_condition, step = self.add_point_on_cross(
                        i, line, ortho_point_dist, not ortho_point_dist["line_point"]
                    )
                else:
                    new_points, break_condition = self.add_point_to_result(
                        self.gps_points[i],
